...
export default class Profile extends React.Component{
  ...
  componentDidMount(){
    axios.post('http://localhost:3002/user/login/profile', {email:localStorage.getItem('email')}).then(res => {
      console.log("resProfile", res.data);
      this.setState({profile:res.data[0]});
      console.log("profile has came", this.state);
    })
...
<div className="timeline_div">
        ...
        <div className="div_name2">{this.state.profile.firstname} {this.state.profile.lastname}</div>
                      </li>
                      <li>
